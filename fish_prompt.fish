...
function fish_prompt -d "Write out the prompt"
    ...
    if test -z "$index"
        set -g __gitprompt $__fish_prompt_normal' on '$__fish_prompt_repo_clean(__fish_git_prompt "%s" | sed 's/ //')
    else
        set -g __gitprompt $__fish_prompt_normal' on '$__fish_prompt_repo_dirty(__fish_git_prompt "%s" | sed 's/ //')
    end
    ...
    echo -n -s \n"$__fish_prompt_userhost" "$__fish_prompt_virtualenv" "in " "$__fish_prompt_cwd" (prompt_pwd) "$__gitprompt" "$__need_push" "$__fish_prompt_normal"\n"$__fish_prompt_char"
